...
(0..(peers.size - 1)).each do
    ...
    puts "tmux new-window -a -t raft_for_beginners:\\$ -n 127.0.0.1:#{this_node} -e 'RUST_LOG=#{log_level}' target/debug/raft_for_beginners -l 127.0.0.1:#{this_node} " + peers_for_this_node.map {|peer_port| "--peer 127.0.0.1:#{peer_port}" }.join(" ")
    peers.rotate!
end
